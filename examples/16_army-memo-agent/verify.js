 ...
 */
{
    ...
    /*
     * The office of record. AR 25-50 publishes no directory of office symbols -
     * para 2-4a(1) says only what one is, and figure B-2 lists the 36 HQDA
     * principal officials by title with no symbols attached. So the directory
     * is the organization's, and nothing ships as a default: an invented
     * office symbol is worse than a blank one.
     */
    {
        const {normalizeOffice, findOffice, officeRecordFields, loadOffices} =
            await import("./offices.js");

        const directory = [
            normalizeOffice({id: "atzb-rc", name: "Range Control", officeSymbol: "ATZB-RC",
            ...
            normalizeOffice({id: "atzb-ja", name: "Staff Judge Advocate", officeSymbol: "ATZB-JA"}),
        ];

        const picked = specFromForm({request: "notify the battalions", office: "atzb-rc"}, directory);
        check("a picked office supplies its own symbol", picked.officeSymbol, "ATZB-RC",
            "AR 25-50, para 2-4a(1)");
        check("and its ARIMS record number", picked.arimsRecordNumber, "25-50a",
            "AR 25-50, para 2-4a(2)");
        check("and its letterhead", picked.letterhead.organization,
            "Headquarters, 4th Infantry Division", "AR 25-50, para 1-18");

        // Only what the entry actually holds.
        const partial = specFromForm({request: "x", office: "atzb-ja"}, directory);
        check("an entry with no letterhead leaves it a placeholder",
            hasPlaceholdersDeep(partial.letterhead), true, "AR 25-50, para 1-18");
        check("while still supplying the symbol it does hold", partial.officeSymbol, "ATZB-JA",
            "AR 25-50, para 2-4a(1)");

        checkTrue("no office picked leaves every record field a placeholder",
            hasPlaceholdersDeep(specFromForm({request: "x"}, directory).officeSymbol),
            "AR 25-50, para 2-4a(1)");
        checkTrue("an unknown office id is not silently accepted",
            hasPlaceholdersDeep(specFromForm({request: "x", office: "nope"}, directory).officeSymbol),
            "AR 25-50, para 2-4a(1)");
        checkTrue("a typed symbol still wins over the directory",
            specFromForm({request: "x", office: "atzb-rc", officeSymbol: "XXXX-YY"}, directory)
                .officeSymbol === "XXXX-YY", "AR 25-50, para 2-4a(1)");

        check("findOffice returns null for nothing", findOffice(directory, ""), null, "directory");
        check("officeRecordFields of nothing is empty", Object.keys(officeRecordFields(null)), [],
            "directory");
        check("a missing directory file is not an error",
            (await loadOffices("/nonexistent/offices.json")).length, 0, "directory");

        // The picker has to reach the page, or it is not a picker.
        const withDirectory = createMemoServer({offices: directory});
        await new Promise((r) => withDirectory.listen(0, r));
        const dirBase = `http://127.0.0.1:${withDirectory.address().port}`;
        const html = await (await fetch(`${dirBase}/`)).text();
        checkTrue("the page renders one option per office",
            directory.every((o) => html.includes(`value="${o.id}"`)), "directory");
        checkTrue("carrying the symbol the option will fill in",
            html.includes('data-symbol="ATZB-RC"'), "AR 25-50, para 2-4a(1)");
        const listed = await (await fetch(`${dirBase}/offices`)).json();
        check("the directory is served for anything else that needs it",
            listed.offices.length, directory.length, "directory");
        await new Promise((r) => withDirectory.close(r));
    }
